...
<body>
	...
	<?php
		...
		$st = "no-drop";
		$bg = "1";
		$anime= $nomor= $name = $email = "$ep isi dulu $ec";
		if (isset($_POST['register'])) {
			...
			if (strlen($nomor) <= 8) {
				$dg = strlen($nomor);
				$nomor = "$ep bohong masa nomor mu $dg digit $ec";
			}
			...
			if (empty($_POST["email"])) {
				...
				if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
					$email = "$ep Salah, coba cek lagi :) $ec";
					$notAllow = $st;
					$bg = "0.4";
	...
	<div class="center">
		...
		<div class="form">
			...
			<div class="form-data">
				...
				<h4>Verifikasi sebelum lanjut...</h4>
				...
				<button class="tombol" style="cursor: <?php echo $notAllow; ?>;filter: brightness(<?php echo $bg ?>);">Lanjut</button>
